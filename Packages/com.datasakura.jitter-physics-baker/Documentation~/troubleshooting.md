...
This page maps messages emitted by DataSakura Jitter Physics Baker `0.0.12` to the smallest
...
| `The prebuilt Jitter2 assembly is missing from the package; run tools~/build-jitter2-unity.sh.` | The package contents are incomplete. | Inspect `Jitter2~/Prebuilt/Jitter2.Core.dll` in the package source. | Reinstall a complete `0.0.12` package; run the named build script only when developing the package itself. |
...
| Exact message | Cause | Check | Fix |
| --- | --- | --- | --- |
| `No artifact asset was supplied.` | The Unity runtime loader received a null artifact reference. | Inspect the consumer component or startup code that calls `JitterPhysicsArtifactLoader.Load`. | Assign the intended `.physics.asset` before startup and fail the scene/match startup while it is null. |
...
| `Building the world failed and was rolled back: {exceptionMessage}` | Jitter2 threw while the static bodies were being created. Created bodies were removed, but earlier world-setting changes were not restored. | Retain the complete typed error and inspect the artifact record/Jitter2 failure that preceded it. | Discard the failed candidate world, correct or replace the artifact/runtime, then build a fresh world before adding gameplay bodies. |
