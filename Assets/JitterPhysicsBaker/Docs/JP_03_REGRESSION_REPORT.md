...
- В follow-up для `0.0.9` по прямому уточнению пользователя `Moved` получил приглушённый
  терракотовый `#A66B5B`, а `Removed` — приглушённый кирпично-красный `#8F4F4A`. `Changed`
  сохранил табачный `#A87945`; подписи и штриховка остаются обязательным различием.
...
- Цветовой follow-up публикуется отдельной версией `0.0.9` без изменения схемы артефакта.
