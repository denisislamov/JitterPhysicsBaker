...
### MT-21. Удаление клиентского артефакта — только текущего

1. В `Bake > Danger zone` нажать `Remove baked physics`, прочитать диалог и отменить.
2. Повторить и подтвердить удаление.

Ожидаемо: в диалоге перечислены ровно три пути удаляемого артефакта; после подтверждения
другие артефакты и экспортированные серверные копии остались на месте; `Client artifact`
показывает `Not baked`, а сохранённый last hash уровня очищен.

### MT-21A. HTTP-загрузка на сервер

1. Запустить WebViewer с `--artifacts <папка>`; для удалённого хоста также передать
   `--upload-token <token>`.
2. В `Settings > Server delivery` указать Base URL и тот же token, вернуться в `Bake`.
3. Нажать `Upload to Server`.

Ожидаемо: сервер повторно проверил payload, manifest, runtime id и канонические имена,
сохранил два файла и окно сообщает, что нужен перезапуск. До перезапуска живой physics
world не изменился. Token не появился ни в сцене, ни в assets, ни в git diff.
...
5. Убедиться, что прежний pose/удалённая форма остаётся видна из сохранённого bake, текущая
   форма имеет штриховку и подпись: `Changed` остаётся табачным, `Moved` использует
   приглушённую терракоту, `Removed` — более глубокий приглушённый кирпично-красный.
...
9. Повторить совместно с Unity Collider gizmos и navigation preview на светлой теме, тёмной
   теме и текстурированном фоне. Проверить, что приглушённые красные оттенки используются
   только для `Moved`/`Removed`, не похожи на яркие Unity error gizmos и не перекрывают
   navigation плотной заливкой.
