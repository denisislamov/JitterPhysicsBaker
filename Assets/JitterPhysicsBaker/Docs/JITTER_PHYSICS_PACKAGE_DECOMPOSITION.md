...
1. **`Jitter2~/Runtime` содержит upstream `2.8.9`, а не EFT-форк.** Это непатченная версия: нет `JITTER_UNITY`. Как Unity fallback он теперь **пригоден**: пакет собирает снапшот сам в `netstandard2.1`-сборку и ставит её плагином, потому что ограничение C# 9 действует на исходники, которые компилирует Unity, а не на сборку, которую она загружает. Пробелы BCL закрыты `Jitter2~/Compat`, интринсики заменены программной реализацией (`intrinsicsProfile: scalar-shim`). Ту же сборку использует сервер — иначе стороны разошлись бы по разным веткам решателя. Детали — `Jitter2~/PATCHES.md` и JP-T02.4.
...
Блокер был подтверждён прогоном в редакторе (Unity 6000.3.19f1, 2026-08-13), а затем **снят** (2026-08-14). Установка непатченного upstream-снапшота **исходниками** действительно невозможна, по двум независимым причинам:

1. **Синтаксис.** Снапшот использует file-scoped namespaces, primary constructors, `scoped`-параметры и константные интерполированные строки. Unity компилирует игровые сборки как C# 9 и **игнорирует** `-langversion` из `csc.rsp` рядом с asmdef — попытка обойти это через `csc.rsp` была реализована, прогнана и не сработала.
2. **BCL.** `Precision.cs`, `TreeBox.cs`, `VertexSupportMap.cs`, `Contact.cs` используют `System.Runtime.Intrinsics.Vector128`, которого нет в .NET Standard 2.1.

Фактическое последствие установки в том прогоне: проект переставал компилироваться целиком, включая Editor-сборку самого пакета, из-за чего окно Setup становилось недоступным, а откат приходилось делать удалением файлов вручную.

**Решение.** Ограничение C# 9 действует на исходники, которые компилирует Unity, но не на сборку, которую она загружает. Пакет собирает снапшот сам (`Jitter2~/StandaloneUnity/Jitter2.Core.csproj`, `netstandard2.1`, современный компилятор) и ставит результат как managed-плагин. Языковая половина проблемы исчезает целиком; оставшиеся пробелы BCL закрыты `Jitter2~/Compat` (программный `Vector128`, `PriorityQueue`, `NativeMemory`, `CollectionsMarshal.AsSpan`, `IsExternalInit`, `SkipLocalsInit`) и 17 локальными правками `tools~/patch-jitter2-netstandard.py`.

Измерено, а не предположено: под `netstandard2.1` + C# 9 снапшот даёт 398 ошибок, при смене только версии языка — 66, после shim и патчей — 0.

Сервер переведён на **ту же** сборку. Это исправление корректности, а не упрощение сборки: Jitter2 держит две реализации решателя контактов и выбирает по `Vector128.IsHardwareAccelerated`, поэтому сервер, собранный из исходников под net10.0, шёл бы по SIMD-ветке, а клиент — по скалярной, и из одного артефакта получились бы две разные симуляции.

`patchSetId` = `unity-netstandard21-v1`, профиль объявляет `intrinsicsProfile: scalar-shim` и `polyfillProfile: netstandard21`; `SupportsUnity` теперь истинно, что зафиксировано в `JitterPhysicsLockTests`. Изменились `sourceContentHash` и `runtimeCompatibilityId` — артефакты прошлых сборок требуют перезапекания.
