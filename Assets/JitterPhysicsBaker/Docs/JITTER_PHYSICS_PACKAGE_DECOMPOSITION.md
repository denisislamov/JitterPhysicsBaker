...
Блокер сохраняется: текущий snapshot — непатченный upstream, его Unity-совместимость не подтверждена (см. отклонение 1 в разделе 0). Установщик об этом предупреждает при каждом запуске. Прогон в редакторе (MT-03) показал ошибки C# 10+ синтаксиса (file-scoped namespaces, коллекционные выражения); установщик теперь кладёт рядом с asmdef `csc.rsp` с `-langversion:latest`, что должно снять их — требуется перепроверка в редакторе.
